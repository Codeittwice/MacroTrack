...
import { estimateMeal, groundEstimate, type GroundedEstimateItem } from '@/lib/ai';
...
export function AiEstimateTab({ date, meal, onLogged }: { date: DateKey; meal: number; onLogged: () => void }) {
  ...
  const estimate = async () => {
    ...
    setStatus(null);
    try {
      const result = await estimateMeal({ provider: settings.aiProvider, apiKey: apiKey ?? '', description });
  ...
  return <div className="flex flex-col gap-4">
    <div><label className="mb-1.5 block text-sm text-muted" htmlFor="meal-description">Meal description</label><Input id="meal-description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="e.g. chicken curry with rice" autoFocus /></div>
    {items.length === 0 && <Button variant="primary" disabled={!description.trim() || !apiKey || estimating} onClick={() => void estimate()}><Sparkles size={18} /> {estimating ? 'Estimating...' : 'Estimate meal'}</Button>}
